...
누구나 React를 처음 배울 때, useState를 쓰면서 왜 배열을 직접 수정하면 안 되는지, 왜 spread operator를 써야 하는지 같은 의문이 생긴다. 그냥 외워서 쓰는 사람도 많긴한데, 일단 React만의 특별한 규칙이 아니라 JavaScript의 데이터 타입 특성과 React의 렌더링 최적화 전략이 만나서 생긴 결과라고 보면 된다.
...
버튼을 클릭하면 count 변수는 증가한다. console.log로 찍어보면 1, 2, 3으로 올라가는 게 보인다. 그런데 막상 화면에는 계속 0으로 표시 될건데,
