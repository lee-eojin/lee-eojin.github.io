...
state는 이 두 가지 문제를 해결한다. 첫째, state는 컴포넌트가 다시 렌더링되어도 값이 유지된다. React가 내부적으로 값을 기억하고 있기 때문이다. 둘째, state가 변경되면 React에게 화면을 다시 그려야 한다고 알려준다. (리렌더링 트리거)
...
여기서 첫 번째 항목을 '포도'로 바꾸고 싶다. 초보자였던 당신(나)은 분명히 직관적으로 이렇게 하고 싶을 것이다.
